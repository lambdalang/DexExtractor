...
void DexHacker::writeEncodedDex2Dex(const char *encodedDex){
    FILE *outFile=fopen(encodedDex,"wb");

    char dexbufferNamed[256]={0};

    strcat(dexbufferNamed,encodedDex);
    strcat(dexbufferNamed,".real.dex");
    remove(dexbufferNamed);
    FILE *file=fopen(dexbufferNamed, "rb");
    ...
    if (file!=NULL) {
      ...
      //  std::cout<<sizeof(char)<<"\nxxxxxxxx"<<fsize;
        ...
        unsigned long  numread =fread(list,sizeof(char),fsize,file);
